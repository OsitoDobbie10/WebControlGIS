...
import { v,UseOperaciones } from "../index";
import { BiCaretLeft } from "react-icons/bi";
import { BiCaretRight } from "react-icons/bi";
import { BiCoinStack } from "react-icons/bi";
const Paginacion = ({ pagina, setPagina, maximo }) => {
  const { bgCategoerias, colorCategoria } = UseOperaciones();
  ...
  return (
    <Container $bgCategoria={bgCategoerias} $colorCategoria={colorCategoria}>
      <button onClick={inicio}>
        <span>{<BiCoinStack />}</span>
      </button>
      <button disabled={pagina === 1 || pagina < 1} onClick={previousPage}>
      <span className="iconoIzquierda">{<BiCaretRight/>}</span>
      </button>
      <span>{input}</span>
